...
async def init_db():
    ...
    # 2. Get the URI as a string
    mongo_uri = str(settings.MONGODB_URI)
    
    # 3. DEFENSIVE FIX: Remove port from SRV connection string if present.
    ...
    # 4. Initialize Client with optimized settings
    ...
    # 5. Initialize Beanie ODM (ONLY ONCE!)
    await init_beanie(
        database=_client.strata_ai, # Using your specific strata_ai database
        document_models=[
            User, 
            FinancialRecord, 
            StartupProfile, 
            UserSettings,
            InvestorLead  # <-- Added your new CRM model right here!
        ]
    )
